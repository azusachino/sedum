# Sedum Architecture Review & Scale Simulation Plan

This document reviews the Sedum personal Markdown wiki skeleton, analyzes popular wikis, simulates scale characteristics (10k to 100k files), and defines crucial architectural push-backs before we write any code.
...
### The Sedum Synthesis
Sedum takes the **ownership and filesystem-as-truth** of Obsidian, combining it with the **server-side indexing power** of a database (Postgres). It rejects client-heavy Electron bloat (Obsidian) and block-level API complexity (Notion) to deliver a server-rendered, thin-client browser wiki.
...
flowchart TD
    ...
    subgraph Axum ["Axum Server (Read-Only on DB)"]
...
- **Action:** Tanaka-san drops 50,000 markdown files into the `sedum/` folder via a script.
- **Workflow Simulation:**
...
- **Action:** Priya opens `postgres-failover.md` in the browser, while a background `git pull` updates the file on disk. Priya attempts to save her edits.
- **Workflow Simulation:**
...
- **Action:** Mei types `[[` in the textarea and starts searching.
- **Workflow Simulation:**
...
- **The Issue:** The planned index schema (`migrations/0001_init_index.sql`) stores `body_tsv` (the tsvector) but **does not store the raw body text** in the database. However, ADR-1 states search snippets will be generated via `ts_headline`.
- **The Conflict:** `ts_headline(config, document, query)` requires the original raw document text to highlight matching terms. If the body text is not in the database, `ts_headline` cannot run in SQL. Reading raw files from disk for every search result in the Axum handler is highly inefficient and creates an I/O bottleneck at scale.
