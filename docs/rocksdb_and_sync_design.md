...
3. **Loss of "Files-are-Truth" Simplicity:** If RocksDB is the primary store, the plain markdown files are no longer the immediate source of truth. Users editing files via terminal or Git must run a manual synchronization command (`miku sync`) to import files into RocksDB before they appear in the browser.
...
flowchart TD
    ...
    subgraph Server ["Miku Rust Process"]
        Axum["Axum Handler"]
        DB[("Operational DB\n(RocksDB/Postgres)")]
...
2. **Background Exporter:** A thread loops periodically (e.g., every 5 seconds). It finds all `DIRTY` pages in the DB, writes them out to their corresponding filesystem paths under `miku/`, and marks them `SYNCED`.
3. **Bulk Importing (One-Way):** If the user updates files externally (e.g., via `git pull`), they trigger `POST /api/sync` or run `miku --sync`. This performs a full directory walk, compares file `mtimes` against the DB, and rebuilds the DB records.
...
- **Path B: RocksDB + Tantivy (Drop Postgres):** We discard Postgres entirely, making Miku a self-contained single-binary with RocksDB (for page storage) and Tantivy (for search index). This eliminates the Postgres installation requirement for users.
